...
// EmitBlockBloomEvent emit block bloom events
func (k Keeper) EmitGasLimitEvent(ctx sdk.Context, gasLimit uint64) {
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			types.EventTypeGasLimit,
			sdk.NewAttribute(types.AttributeKeyEthereumGasLimit, fmt.Sprint(gasLimit)),
		),
	)
}
